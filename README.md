...
> [!WARNING]
> O suporte padrão do Windows 10 terminou em **14 de outubro de 2025**. O sistema continua funcionando, mas a Microsoft recomenda migrar para Windows 11 quando o dispositivo for compatível ou avaliar opções de ESU. Este projeto não substitui atualizações de segurança do sistema operacional.
...
1. Baixe `scripts/OHenrique-Optimization-Windows-10.ps1`.
2. Abra **Windows PowerShell/Terminal como Administrador**.
3. Execute:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\OHenrique-Optimization-Windows-10.ps1"
```
...
├── docs/
├── scripts/
│   └── OHenrique-Optimization-Windows-10.ps1
├── CHANGELOG.md
├── CONTRIBUTING.md
├── DISCLAIMER.md
├── LICENSE
