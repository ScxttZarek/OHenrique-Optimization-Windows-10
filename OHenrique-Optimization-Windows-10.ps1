...
# OHenrique Optimization - Windows 10
# Version: 1.0.0
...
# Windows 10 Universal Edition
# Notebooks & Desktops
# ============================================================

$ErrorActionPreference = "Continue"
$Host.UI.RawUI.WindowTitle = "OHenrique Optimization | Windows 10 Universal"
...
function Banner {
    ...
    Write-Host ""
    Write-Host "          O H E N R I Q U E   O P T I M I Z A T I O N" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "                 WINDOWS 10 | UNIVERSAL EDITION" -ForegroundColor White
...
if (-not $Admin) {
    ...
    Write-Host ""
    Write-Host " Abra o menu Iniciar > Windows PowerShell > Executar como administrador." -ForegroundColor Yellow
    Write-Host ""
...
try {
    ...
    if ($OS.Caption -notlike "*Windows 10*") {
        ...
        Warn "Esta edicao foi preparada especificamente para Windows 10."
        $Continuar = Read-Host "Continuar mesmo assim? [S/N]"
        if ($Continuar -notmatch "^[SsYy]") { return }
    } else {
        OK "Windows 10 detectado."
    }
}
catch {
...
$Log = Join-Path $Desktop ("OHenrique_Windows10_" + (Get-Date -Format "yyyy-MM-dd_HH-mm-ss") + ".log")
...
try {
    ...
    Warn "Nao foi possivel iniciar o log."
}

Separator
Write-Host " INFORMACOES DO COMPUTADOR" -ForegroundColor Cyan
Write-Host ""
...
Separator
Write-Host " CONFIGURACAO DA OTIMIZACAO" -ForegroundColor Cyan
Write-Host ""
...
function Etapa {
    ...
    Write-Progress -Activity "OHenrique Optimization | Windows 10" `
        -Status "Etapa $script:Atual de $script:Total - $Nome" `
        -PercentComplete $Percent

    Separator
    Write-Host " [$script:Atual/$script:Total] $Nome" -ForegroundColor Cyan
    Write-Host ""
...
Etapa "Criando ponto de restauracao" {
    try {
        Checkpoint-Computer -Description "OHenrique Optimization Windows 10" `
...
Etapa "Executando Limpeza de Disco" {
...
Etapa "Limpando cache do Delivery Optimization" {
    try {
        ...
        if (Get-Command Delete-DeliveryOptimizationCache -ErrorAction SilentlyContinue) {
            ...
            Info "Funcao nao disponivel nesta instalacao. Etapa ignorada."
        }
    } catch {
        Warn "Delivery Optimization nao pode ser limpo."
    }
}
...
Etapa "Otimizando unidades locais" {
    $LocalDisks = Get-CimInstance Win32_LogicalDisk | Where-Object { $_.DriveType -eq 3 -and $_.FileSystem }
...
Separator
Write-Host " EXTRA | LIXEIRA" -ForegroundColor Cyan
if ($LimparLixeira) {
...
Separator
Write-Host " EXTRA | ATUALIZACAO DE APLICATIVOS" -ForegroundColor Cyan
if ($AtualizarApps) {
...
Write-Progress -Activity "OHenrique Optimization | Windows 10" -Completed
